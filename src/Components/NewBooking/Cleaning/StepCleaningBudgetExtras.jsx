// src/Components/Booking/Cleaning/StepCleaningBudgetExtras.jsx
...
const StepCleaningBudgetExtras = ({
  visible,
  onBack,
  onNext,
  ...
  resBudget,
  setResBudget,
  dueDate, // поки не використовуємо
  setDueDate,
  extraDetails,
  setExtraDetails,
  ...
  // 🔒 Безпечні масиви — навіть якщо пропси undefined
  ...
  if (isResidential) {
    canContinue =
      safeAreas.length > 0 &&
      safeGeneralTasks.length > 0 &&
      safeKitchenTasks.length > 0 &&
      !!resBudget;
  } else if (isCommercial) {
    canContinue = !!comBudget;
  }
                ...
                {GENERAL_TASKS_OPTIONS.map((name) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      className="w-4 h-4 rounded border-[#D4D4D8]"
                      checked={safeGeneralTasks.includes(name)}
                      onChange={() =>
                ...
                {KITCHEN_TASKS_OPTIONS.map((name) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      className="w-4 h-4 rounded border-[#D4D4D8]"
                      checked={safeKitchenTasks.includes(name)}
                      onChange={() =>
...
export default StepCleaningBudgetExtras;
